...
from algorithms.ac.model import Model
...
class Predictor(Predictor_Base):
    ...
    def _init_sess(self):
        ...
        with self.graph.as_default():
            self.feature = tf.placeholder(shape=(None, self.state_dim[0], self.state_dim[1], \
                    self.state_dim[2]), name="feature", dtype=np.uint8)
