...
class Predictor(Predictor_Base):
    ...
    def process(self, feature):
        if feature['pred_style']:
            long_style_loss, short_style_loss, style_long_pred, style_short_pred = \
                self.sess.run([self.model.long_style_loss,
                               self.model.short_style_loss,
                               self.model.pred_long_term_style,
                               self.model.pred_short_term_style],
                              feed_dict={self.feature: feature['state'],
                                         self.style_input: feature['style_input'],
                                         })
            return long_style_loss, short_style_loss, style_long_pred, style_short_pred
        else:
